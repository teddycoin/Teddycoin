...
const string strMessageMagic = "Teddycoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Teddycoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Teddycoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
//int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nFees, int nHeight)
{
    ...
    nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE;
			...
			if(pindexBest->nHeight < YEARLY_BLOCKCOUNT)
				nRewardCoinYear = 20 * MAX_MINT_PROOF_OF_STAKE; //first year 20%
			else if(pindexBest->nHeight < (2 * YEARLY_BLOCKCOUNT))
				nRewardCoinYear = 15 * MAX_MINT_PROOF_OF_STAKE; // year 2 15%
			else if(pindexBest->nHeight < (3 * YEARLY_BLOCKCOUNT))
				nRewardCoinYear = 10 * MAX_MINT_PROOF_OF_STAKE; //year 3 10%
			else if(pindexBest->nHeight < (4 * YEARLY_BLOCKCOUNT))
				nRewardCoinYear = 8 * MAX_MINT_PROOF_OF_STAKE; //year 4 8%
...
// Teddycoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes Teddycoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Teddycoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Teddycoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Teddycoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Teddycoin: track money supply and mint amount info
...
// Teddycoin: total coin age spent in transaction, in the unit of coin-days.
...
// Teddycoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // Teddycoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Teddycoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Teddycoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // Teddycoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // Teddycoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Teddycoin: check proof-of-stake
    ...
    // Teddycoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Teddycoin: getblocks may not obtain the ancestor block rejected
    ...
    // Teddycoin: if responsible for sync-checkpoint send it
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    {
        if (wallet.TeddycoinStake(wallet, nBits, nSearchTime-nLastCoinStakeSearchTime, nFees, txCoinStake, key))
        {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Teddycoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "TeddyCoin is X13 POW-POS -----------and yes";
        CTransaction txNew;
        txNew.nTime = 1404208528;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404208528;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 6957372;
        if(fTestNet)
        {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("565623867ecc77cd9ebe41a5635b2bcfcbbd2a879c58882255900dd7b122ba15"));
        block.print();
        ...
        // Teddycoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Teddycoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Teddycoin: tell downloading node about the latest block if it's
